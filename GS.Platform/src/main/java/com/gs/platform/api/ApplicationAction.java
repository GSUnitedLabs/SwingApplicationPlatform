 ...
 *  Author     :   Sean Carrick
 *  Created    :   Oct 22, 2021 @ 9:03:21 PM
...
/**
 * The {@link javax.swing.Action} class used to implement the `@Action`
 * annotation. This class is typically not instantiated directly, it's created
 * as a side effect of constructing an `ApplicationActionMap`:
 * <pre>
 ...
 * properties from the specified `ResourceMap}. Resource names must match the
 * `@Action`'s name, which is the name of the corresponding method, or the value
 * of the optional `@Action` name parameter. To initialize the text and
 * shortDescription properties of the action named `"anAction"} in the previous
 * example, one would define two resources:
 * <pre>
 ...
 * can be found in the ApplicationAction {@link
 * #ApplicationAction constructor} documentation.
 *
 * <p>
 * An ApplicationAction's `enabled} and `selected} properties can be delegated
 * to boolean properties of the Actions class, by specifying the corresponding
 * property names. This can be done with the `@Action` annotation, e.g.:
 * <pre>
 * public class MyActions {
 ...
 *     }
 * }
 * </pre> If the MyActions class supports PropertyChange events, then then
 ...
 * `Task.BlockingScope.ACTION}, then the action will be disabled while the
 * actionPerformed method runs.
 *
 * <p>
 * An ApplicationAction can have a `proxy} Action, i.e. another Action that
 * provides the `actionPerformed` method, the enabled/selected properties, and
 * values for the Action's long and short descriptions. If the proxy property is
 * set, this ApplicationAction tracks all of the aforementioned properties, and
 * the `actionPerformed` method just calls the proxy's `actionPerformed` method.
 * If a `proxySource} is specified, then it becomes the source of the
 * ActionEvent that's passed to the proxy `actionPerformed` method. Proxy action
...
public class ApplicationAction extends AbstractAction {
     ...
     * values of resources whose key begins with `baseName}. ResourceMap keys
     * are created by appending an &#064;Action resource name, like
     ...
     * <ul>
     * <li>`Action.text}<br>
     * Used to initialize the Action properties with keys `Action.NAME},
     * `Action.MNEMONIC_KEY} and `Action.DISPLAYED_MNEMONIC_INDEX}. If the
     * resources's value contains an "&amp;" or an "_" it's assumed to mark the
     * following character as the mnemonic. If
     ...
     *
     * <li>`Action.displayedMnemonicIndexKey}<br>
    ...
    /**
     * The name of the `@Action` enabledProperty whose value is returned by
     * {@link #isEnabled isEnabled}, or null.
     *
    ...
    /**
     * The name of the `@Action` selectedProperty whose value is returned by
     * {@link #isSelected isSelected}, or null.
     *
     ...
     * <ul>
     * <li>`actionPerformed`<br>
     * Our `actionPerformed` method calls the delegate's after the ActionEvent
     * source to be the value of `getProxySource`</li>
     ...
     * </ul>
     *
     * @param proxy the `javax.swing.Action` to set the proxy
     * 
     * @see #setProxy
     * @see #setProxySource
     * @see #actionPerformed
     ...
     * corresponding `@Action` method (unless the `name` &#064;Action parameter
     * was specified).
     *
     * <p>
     ...
     * `ApplicationActionMap`s.
     *
     * <p>
     * Note: this property should not confused with the {@link
     * javax.swing.Action#NAME Action.NAME} key. That key is actually used to
     * initialize the `text} properties of Swing components, which is why we
     * call the corresponding ApplicationAction resource "Action.text", as in:
     * <pre>
 ...
 values are selected based exclusively on their type:
    <table border=1>
    <caption>@Action Parameter Types &amp; Values</caption>
    <tr>
    ...
    <tr>
    <td>`ResourceMap}</td>
    <td>the `ResourceMap} of the the `ActionMap` that contains this
 `Action`</td>
    </tr>
    <tr>
 ...
 <p>
     ...
     * the `Action.Parameter} annotation, which is passed along as the `pKey}
     ...
     * @return the argument to an `@Action` annotation
     */
    ...
    /**
     * If the proxy action is null and `enabledProperty} was specified, then
     * return the value of the enabled property's is/get method applied to our
     * ApplicationActionMap's `actionsObject`. Otherwise return the value of
     * this Action's enabled property.
     *
     * @return {@inheritDoc}
     * @see #setProxy
     * @see #setEnabled
    ...
    /**
     * If the proxy action is null and `enabledProperty} was specified, then set
     * the value of the enabled property by invoking the corresponding `set}
     * method on our `ApplicationActionMap`'s `actionsObject`. Otherwise set the
    ...
    /**
     * If the proxy action is null and `selectedProperty} was specified, then
     * return the value of the selected property's is/get method applied to our
     * ApplicationActionMap's `actionsObject`. Otherwise return the value of
     * this Action's enabled property.
     *
    ...
    /**
     * If the proxy action is null and `selectedProperty} was specified, then
     * set the value of the selected property by invoking the corresponding
     * `set} method on our ApplicationActionMap's `actionsObject`. Otherwise set
     * the value of this Action's selected property.
     *
    ...
    /**
     * Keeps the `&#064;Action selectedProperty} in sync when the value of `key}
    ...
    /**
     * Returns a string representation of this `ApplicationAction} that should
     * be useful for debugging. If the action is enabled it's name is enclosed
     * by parentheses; if it's selected then a "+" appears after the name. If
     * the action will appear with a text label, then that's included too. If
     * the action has a proxy, then we append the string for the proxy action.
     *
