 ...
 *  Author     :   Sean Carrick
 *  Created    :   Oct 22, 2021 @ 8:53:45 PM
...
final class DefaultInputBlocker extends Task.InputBlocker {
    ...
    private static final LogRecord record = new LogRecord(DefaultInputBlocker.class.getSimpleName());
    ...
    private void showBlockingDialog(boolean f) {
        if (f) {
            if (modalDialog != null) {
                String msg = String.format("unexpected InputBlocker state [%s] %s",
                        f, this);
                record.setInstant(Instant.now());
                record.setMessage(msg);
                record.setParameters(new Object[] {f});
                record.setSourceMethodName("showBlockingDialog");
