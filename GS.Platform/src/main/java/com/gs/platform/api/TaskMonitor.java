 ...
 *  Author     :   Sean Carrick
 *  Created    :   Oct 22, 2021 @ 8:20:52 PM
 ...
 * well as the state of a single `foreground} Task.
 *
 * <p>
 * The value of {@link #getTasks getTasks()} is a list of all of the `Tasks}
 * whose state is not {@link
 * Task#isDone DONE} for all of the ApplicationContext's `TaskServices}. In
 * other words: all of the ApplicationContext's background tasks that haven't
 ...
 * TaskMonitor's PropertyChangeListener is notified each time a property of the
 ...
 * "pending", "started", and "done" when the corresponding Task `state} property
 * changes occur.
 *
 * <p>
 ...
 * reset automatically, one must set {@link
...
public class TaskMonitor extends AbstractBean {
     ...
     *
     * @param context the `ApplicationContext` in which this `TaskMonitor` is
     * being used
     */
     ...
     * property of the the `foregroundTask} changes. By default this property is
     * set to the first Task to be executed and then, when that Task finishes,
     * reset to the next most recently executed Task. If `@Action`'s
    ...
    /**
     * Indicates the `Task} whose status the ApplicationContext's GUI wants to
     * be displayed, typically in the main window's status bar.
     *
     *
    ...
    /**
     * True if the `foregroundTask} property should be automatically reset to
     * the oldest Task in the queue when it finishes running.
     * <p>
    ...
    /**
     * True if the `foregroundTask} property should be automatically reset to
     * the oldest Task in the queue when it finishes running. An application
     ...
     *
     * @return a list of all Tasks that aren't `DONE}
     */
